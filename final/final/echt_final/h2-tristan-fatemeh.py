# visualize_hypothesis.py - Tristan (movie_id schema)
...
def build_dataset():
    """
    Build a compact dataset for plots, using the new movie_id-based schema:
      - public.movie(movie_id, url, title, reldate)
      - public.distribution(movie_id, theatre_count, opening_weekend_revenue, ...)
      - public.performance(movie_id, production_budget, worldwide_box_office, opening_weekend_revenue)
      - public.expert_reviews(movie_id, date, idvscore, ...)
    Critics are averaged per movie_id (no date window here).
    """
    sql = """
    WITH critics AS (
        SELECT
    ...
    SELECT
        ...
        m.reldate,
        d.theatre_count,
        COALESCE(d.opening_weekend_revenue, p.opening_weekend_revenue) AS opening_weekend_revenue,
        p.production_budget,
        c.criticscore_mean,
        c.critics_n
    FROM public.movie m
    ...
    # Keep only the columns we need for the rest (tidy DataFrame)
    cols = [
        "movie_id", "url", "title", "reldate",
...
def partial_corr(df, x, y, z):
    ...
    rx = sm.OLS(d[x], Xz).fit().resid   # residuals of x after removing z
    ry = sm.OLS(d[y], Xz).fit().resid   # residuals of y after removing z
...
def visualize(df):
    ...
    if m1 is not None:
        ...
        print("\n[warn] Not enough rows for OLS: log(Theatre count)")

    if m2 is not None:
        print("\n=== OLS: log(Opening weekend) ~ critics + log(budget) ===")
        print(m2.summary())
    else:
        print("\n[warn] Not enough rows for OLS: log(Opening weekend)")
