...
def get_distribution() -> pd.DataFrame:
    """
    Haal distribution op. Als 'avg_run_per_theatre' niet bestaat,
    bereken dan een fallback: (COALESCE(d.OWR, p.OWR) / theatre_count).
    """
    ...
    if has_avg:
        ...
        SELECT
          ...
          d.opening_weekend_revenue::numeric             AS opening_weekend_revenue,
        ...
        # Fallback: gebruik evt. p.opening_weekend_revenue als d.* ontbreekt; vermijd deling door 0
        sql = """
        SELECT
          d.title,
          d.reldate,
          ...
          (COALESCE(d.opening_weekend_revenue, p.opening_weekend_revenue)::numeric
           / NULLIF(d.theatre_count::numeric, 0))                        AS avg_run_per_theatre
        FROM public.distribution d
        LEFT JOIN public.performance p USING (title, reldate)
        WHERE d.theatre_count IS NOT NULL
...
def get_expert_reviews_windowed(days_before:int=7, days_after:int=3) -> pd.DataFrame:
    """
    Aggregate critic scores per movie URL within [reldate - days_before, reldate + days_after].
    """
    sql = """
    SELECT
      er.url,
...
def get_user_reviews_windowed(days_start_after:int=3, days_end_after:int=28) -> pd.DataFrame:
    """
    Aggregate user scores per movie URL within (reldate + days_start_after, reldate + days_end_after].
    """
    sql = """
    SELECT
      ur.url,
...
def build_dataset_h3(
    ...
    # Join path: movie -> distribution/performance by (title, reldate), reviews by url
    df = (m
    ...
    # Construct sustaining proxy: multiplier after opening
...
def print_and_plot_corr_matrices(df: pd.DataFrame):
    ...
    # Quick “which aligns more with critic score re: opening” print
