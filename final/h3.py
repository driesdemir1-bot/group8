...
# ---- DB connection (use a real role on your machine) ----
# If your role == your macOS username, this likely works without a password:
...
def build_dataset_h3(
    ...
    # Clean / keep positives for logs & ratios
    ...
    # (worldwide / opening_weekend); guard zeros
    df = df[df["opening_weekend_revenue"] > 0]
    ...
    # Final clean of logs (drop rows where logs are missing)
    needed = [
    ...
    # Not all rows will have both critic and user windows; keep rows where each block can be computed
    # We'll compute each correlation block on its own subset.
    return df
